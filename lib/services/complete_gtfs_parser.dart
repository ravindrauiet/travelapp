...
/// Complete GTFS Parser that uses stop_times.txt to get all stations
class CompleteGTFSParser {
  static List<MetroStation>? _stations;

  /// Load all metro stations from GTFS data using stop_times.txt
  static Future<List<MetroStation>> getAllStations() async {
    if (_stations != null) return _stations!;

    try {
      print('CompleteGTFS: Loading GTFS data...');
      ...
      // Load stop_times data to get all stations that are actually served
      ...
      // Convert served stops to stations
      _stations = <MetroStation>[];
      for (final stopId in servedStopIds) {
        if (stopsMap.containsKey(stopId)) {
          final stopData = stopsMap[stopId]!;
          final stationName = stopData['name']!;
          ...
          final station = MetroStation(
            ...
            latitude: latitude,
            longitude: longitude,
            facilities: _getStationFacilities(stationName),
            isInterchange: _isInterchangeStation(stationName),
            interchangeLines: _getInterchangeLines(stationName),
          );
          _stations!.add(station);
        }
      }
      
      print('CompleteGTFS: Successfully converted ${_stations!.length} stations');
      return _stations!;
    } catch (e) {
      print('CompleteGTFS: Error loading GTFS data: $e');
      return [];
    }
  }
  ...
  /// Determine line name based on station name - comprehensive mapping
  static String _getLineNameFromStation(String stationName) {
    ...
    if (stationName.contains('Rithala') ||
        ...
        stationName.contains('Mundka') ||
        stationName.contains('Rajdhani Park') ||
        stationName.contains('Nangloi Railway Station') ||
        stationName.contains('Nangloi') ||
        stationName.contains('Maharaja Surajmal Stadium') ||
        stationName.contains('Udyog Nagar') ||
        stationName.contains('Peera Garhi') ||
        stationName.contains('Paschim Vihar') ||
        stationName.contains('Madipur') ||
        stationName.contains('Shivaji Park') ||
        stationName.contains('Punjabi Bagh') ||
        stationName.contains('Ashok Park Main') ||
        stationName.contains('Satguru Ram Singh Marg') ||
        stationName.contains('Samaypur Badli') ||
    ...
    if (stationName.contains('Dwarka') ||
        ...
        stationName.contains('ITO') ||
        stationName.contains('Janpath') ||
        stationName.contains('Khan Market') ||
        stationName.contains('Jawahar Lal Nehru Stadium') ||
        stationName.contains('Jangpura') ||
        stationName.contains('Lajpat Nagar') ||
        stationName.contains('Moolchand') ||
        stationName.contains('Kailash Colony') ||
        stationName.contains('Nehru Place') ||
        stationName.contains('Kalkaji Mandir') ||
        stationName.contains('Govind Puri') ||
        stationName.contains('Harkesh Nagar Okhla') ||
        stationName.contains('Jasola-Apollo') ||
        stationName.contains('Sarita Vihar') ||
        stationName.contains('Mohan Estate') ||
        stationName.contains('Tughlakabad Station') ||
        stationName.contains('Badarpur Border') ||
        ...
        stationName.contains('Mayur Vihar Pocket 1') ||
        stationName.contains('Noida Sec-34') ||
        stationName.contains('Noida Sec-52') ||
        stationName.contains('Noida Sec-61') ||
        stationName.contains('Noida Sec-59') ||
        stationName.contains('Noida Sec-62')) {
      return 'Blue Line';
    }
    ...
    if (stationName.contains('Brigadier Hoshiyar Singh') ||
        ...
        stationName.contains('Nawada') ||
        stationName.contains('Dwarka Mor') ||
        stationName.contains('Dwarka Sector - 14') ||
        stationName.contains('Dwarka Sector - 13') ||
        stationName.contains('Dwarka Sector - 12') ||
        stationName.contains('Dwarka Sector - 11') ||
        stationName.contains('Dwarka Sector - 10') ||
        stationName.contains('Dwarka Sector - 9') ||
        stationName.contains('Dwarka Sector - 8') ||
        stationName.contains('Dwarka Sector - 21')) {
      return 'Green Line';
    }
    ...
    if (stationName.contains('Raja Nahar Singh') ||
        ...
        stationName.contains('Mandi House') ||
        stationName.contains('ITO') ||
        stationName.contains('Delhi Gate') ||
        stationName.contains('Jama Masjid') ||
        stationName.contains('Lal Quila')) {
      return 'Violet Line';
    }
    ...
    if (stationName.contains('Rapid Metro') ||
        ...
        stationName.contains('Sikanderpur (Rapid Metro)')) {
      return 'Rapid Metro';
    }
